...
struct FpsColor {
	public Color _color;
	public int _minimumFps;
}

[RequireComponent(typeof(FpsCounter))]
public class FpsDisplay : MonoBehaviour {

	public Text _highestfpsLabel,_averagefpsLabel,_lowestfpsLabel;
	FpsCounter _fpsCounter;

	[SerializeField]
	private FpsColor[] _coloring;

	static readonly string[] _stringsFrom00To99 = {
	...
	void Display(Text label, int fps) {
		...
		for (int i=0;i <_coloring.Length; i++){
			if (fps >= _coloring[i]._minimumFps) {
				label.color = _coloring[i]._color;
				break;
			}
		}
	}
